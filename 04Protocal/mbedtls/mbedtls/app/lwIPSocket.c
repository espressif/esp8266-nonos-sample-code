...
netconn_alloc(netconn_type type, void *arg)
{
    ...
    netconn->readbuf = ringbuf_new(tcp_sndbuf(pcb));
    lwIP_REQUIRE_ACTION(netconn->readbuf, exit, ret = ERR_MEM);
    netconn->writebuf = ringbuf_new(tcp_sndbuf(pcb));
    lwIP_REQUIRE_ACTION(netconn->writebuf, exit, ret = ERR_MEM);
    system_overclock();
    netconn->socket = -1;
exit:
    if (ret != ERR_OK)
...
static err_t recv_tcp(void *arg, struct tcp_pcb *pcb, struct pbuf *p, err_t err)
{
    ...
    if (p!= NULL)
    {
        ...
        if (newconn->readbuf != NULL)
        {
            ...
            {
                if (ringbuf_is_full(newconn->readbuf))
                {
                    ESP_LOG("No enough buffer for get the data\n");
                    pbuf_is_empty =false;
                    break;
                }
                else
                {
                    bytes_free = ringbuf_bytes_free(newconn->readbuf);
                    if (bytes_free > pthis->len)
                    {
                        bytes_free = pthis->len;
                    }
                    else
                    {
                        pbuf_is_empty = false;
                    ...
                    ringbuf_memcpy_into(newconn->readbuf, pthis->payload, bytes_free);
                    tcp_recved(newconn->tcp, bytes_free);
                }
    ...
    {
        newconn->state = NETCONN_STATE_CLOSED;
...
static err_t sent_tcp(void *arg, struct tcp_pcb *pcb, u16_t len)
{
    ...
    {
        size_t bytes_used = ringbuf_bytes_used(conn->writebuf);
        if (bytes_used != 0)
        {
            ESP_LOG("%s %d, length =%d\n", __FILE__, __LINE__, len);
            lwip_send(find_socket(conn), NULL, bytes_used, 0);
        }
        else
        {
            ESP_LOG("%s %d, length =%d\n", __FILE__, __LINE__, len);
...
static void err_tcp(void *arg, err_t err)
{
    ...
    socket = find_socket(conn);
    lwIP_EVENT_PARSE(socket, err);
    sock = get_socket(socket);
    remove_tcp(conn);
    free_socket(sock);
    free_netconn(conn);
    os_printf("socket TCP error\n");
...
static err_t do_connected(void *arg, struct tcp_pcb *pcb, err_t err)
{
    ...
    conn->state = NETCONN_STATE_ESTABLISHED;
    conn->readbuf = ringbuf_new(tcp_sndbuf(pcb));
    lwIP_REQUIRE_ACTION(conn->readbuf, exit, err = ESP_MEM);
    conn->writebuf = ringbuf_new(tcp_sndbuf(pcb));
    lwIP_REQUIRE_ACTION(conn->writebuf, exit, err = ESP_MEM);
...
static void close_tcp(void *arg, err_t err)
{
	extern void mbedtls_close_tcp(void *arg, err_t err);

	espconn_kill_oldest_pcb();
	mbedtls_close_tcp(arg, err);
}
...
int lwip_send(int s, const void *data, size_t size, int flags)
{
    ...
    bytes_used = ringbuf_bytes_used(sock->conn->writebuf);

    if (bytes_used != 0)
    {
        buff = (void*)os_zalloc(bytes_used);
        if (buff == NULL)
            return -1;
        ringbuf_memcpy_from(buff, sock->conn->writebuf, bytes_used);
    }
    else
    {
        return bytes_used;
    ...
    if (tcp_sndbuf(sock->conn->tcp) < bytes_used)
    {
    ...
    do
    {
        Err = tcp_write(sock->conn->tcp, buff, size, 1);
        if (Err == ERR_MEM)
            size /= 2;
    }
    ...
    if (Err == ERR_OK)
    {
        ...
        {
            bytes_used -= size;
            ringbuf_memcpy_into(sock->conn->writebuf, (void*)((uint8*)buff + size), bytes_used);
        }
    ...
    if (sock->conn != NULL)
    {
        switch (sock->conn->state)
        {
            case NETCONN_STATE_ESTABLISHED:
                if (ringbuf_is_full(sock->conn->writebuf))
                {
                    ESP_LOG("No enough buffer for write data\n");
                    return 0;
                }
                else
                {
                    bytes_free = ringbuf_bytes_free(sock->conn->writebuf);
                    if (size > bytes_free)
                    {
                        size = bytes_free;
                    }
                    ringbuf_memcpy_into(sock->conn->writebuf, data, size);
                }
                lwip_send(s, NULL, size, 0);
                return size;
            default:
                return -1;
        }
